...
def add_word():
        f=0
        new_word_english = input('Please enter new word in english: ')
        for i in range(len(WORDS)):
                if WORDS[i]['english'] == new_word_english :
                        print('This word is exist in translate file.')
                        f=0
                else:
                        new_word_persian = input('Please enter new word in persian: ')
                        dict = {}
                        dict['english'] = new_word_english
                        dict['persian'] = new_word_persian
                        WORDS.append(dict)
                        write_to_file()
                        break
                if f == 0:
